...
def get_embedding(text: str) -> np.ndarray:
    ...
    print(
        f"❌ CACHE MISS: No embedding found for text hash {text_hash[:8]}... (text preview: '{text[:50]}{'...' if len(text) > 50 else ''}')"
    )
    ...
    try:
        ...
        _save_cache(cache)
        print(
            f"💾 CACHED: Saved new embedding for text hash {text_hash[:8]}... (embedding dim: {len(embedding_vector)})"
        )
...
def get_cache_stats() -> dict:
    ...
    return {"cached_embeddings": len(cache), "cache_file_size": cache_size, "cache_file_path": CACHE_PATH}
...
def rerank(query: str, papers: list[Paper], user_feedbacks: list[float] | None = None) -> list[Paper]:
    """
